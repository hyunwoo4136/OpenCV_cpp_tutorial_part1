...
int main(int, char**)
{
    ...
    {
        approxPolyDP(Mat(contours[i]), approx, arcLength(Mat(contours[i]), true) * 0.005, true);    // 컨투어 직선으로 근사화

        int size = approx.size();               
        line(img_result, approx[0], approx[approx.size() - 1], Scalar(0, 255, 0), 3);   // 도형에 컨투어 출력
        for (int k = 0; k < size - 1; k++)
            line(img_result, approx[k], approx[k + 1], Scalar(0, 255, 0), 3);

        if (isContourConvex(Mat(approx)))                                       // 도형 판정 및 라벨 출력
        {
            if (size == 3)
